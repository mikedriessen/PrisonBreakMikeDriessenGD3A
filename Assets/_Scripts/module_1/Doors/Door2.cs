...
public class Door2 : MonoBehaviour, IInteractable
{
    ...
    void Update()
    {
        if (open && transform.rotation.eulerAngles.z < initialRotation +190)
        {
            transform.rotation = Quaternion.RotateTowards(transform.rotation, Quaternion.Euler(-90, 0, initialRotation +190), 5);
